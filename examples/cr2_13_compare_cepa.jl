...
e_ci, v_bst = FermiCG.ci_solve(ci_vector, cluster_ops, clustered_ham);
@printf(" BST CI energies: %s\n", string(e_ci))

GC.gc()
baseline_bst = Base.gc_live_bytes()
r_bst, peak_bst = with_peak_memory() do
    @timed FermiCG.do_fois_cepa(v_bst, cluster_ops, clustered_ham;
                                 max_iter     = 200,
                                 cepa_shift   = "cepa",
                                 cepa_mit     = 30,
                                 nbody        = 4,
...
println("="^W)
ci_tpsci = FermiCG.TPSCIstate(clusters, ref_fock, R=nroots)
ci_tpsci = FermiCG.add_spin_focksectors(ci_tpsci)
GC.gc()
baseline_tpsci = Base.gc_live_bytes()
r_tpsci, peak_tpsci = with_peak_memory() do
    @timed FermiCG.do_fois_cepa(ci_tpsci, cluster_ops, clustered_ham;
                                 cepa_shift   = "cepa",
                                 thresh_foi   = thresh_foi,
                                 nbody        = 4,
                                 tol          = 1e-8,
